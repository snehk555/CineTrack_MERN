...
import { signupSchema, SignupFormData } from '../../../shared/schemas/auth.schema';
...
const SignupPage: React.FC = () => {
  ...
  const signupMutation = useMutation({
    mutationFn: (data: SignupFormData) =>
      apiClient.post('/auth/signup', { username: data.username, email: data.email, password: data.password }),
    onSuccess: (response) => {
  ...
  return (
    <div className="flex justify-center items-center min-h-[85vh] py-8">
      <div className="bg-[#1e1b2e]/90 p-8 rounded-2xl w-full max-w-md border border-violet-900/30 shadow-2xl">
        ...
        <p className="text-slate-400 text-center text-sm mb-8 font-medium">Join CineTrack to build your digital diary</p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            ...
            <input
              type="text"
              placeholder="cinelover99"
              {...register("username")}
              className={`px-4 py-3 rounded-xl bg-slate-900/50 border ${errors.username ? 'border-red-500 focus:border-red-500' : 'border-white/10 focus:border-violet-500'} text-white placeholder-slate-500 focus:outline-none focus:bg-slate-900 transition-all font-medium`}
            />
            {errors.username && <span className="text-red-400 text-xs mt-1 font-medium">{errors.username.message}</span>}
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              className={`px-4 py-3 rounded-xl bg-slate-900/50 border ${errors.confirmPassword ? 'border-red-500 focus:border-red-500' : 'border-white/10 focus:border-violet-500'} text-white placeholder-slate-500 focus:outline-none focus:bg-slate-900 transition-all font-medium`}
            />
            {errors.confirmPassword && <span className="text-red-400 text-xs mt-1 font-medium">{errors.confirmPassword.message}</span>}
          </div>

          <button
            type="submit"
            disabled={signupMutation.isPending}
            className="mt-6 bg-violet-600 text-white font-bold py-3.5 px-4 rounded-xl transition-all duration-300 hover:bg-violet-700 hover:-translate-y-0.5 hover:shadow-[0_4px_20px_rgba(139,92,246,0.3)] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0 disabled:hover:shadow-none"
          >
        ...
        <p className="text-center mt-8 text-sm text-slate-400 font-medium">
          ...
          <Link to="/login" className="text-violet-400 no-underline font-bold ml-1.5 hover:text-violet-300 transition-colors">
            Log In
          </Link>
        </p>
      </div>
