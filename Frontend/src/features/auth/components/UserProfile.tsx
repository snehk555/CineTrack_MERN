  ...
  const handleLogout = async () => {
    ...
    return (
        <div className="relative" ref={profileRef}>
            <div
                className="w-[38px] h-[38px] rounded-full bg-gradient-to-br from-amber-500 to-zinc-500 text-white text-base font-bold flex items-center justify-center cursor-pointer select-none border-2 border-amber-500/50 shadow-[0_0_12px_rgba(139,92,246,0.4)] transition-all duration-200 hover:scale-110 hover:shadow-[0_0_20px_rgba(139,92,246,0.7)]"
            ...
            {isProfileOpen && (
                <div className="absolute top-[calc(100%+12px)] right-0 w-[280px] bg-[#1e1e2e] border border-white/10 rounded-2xl shadow-[0_16px_48px_rgba(0,0,0,0.5)] overflow-hidden z-[1000]" style={{animation: 'fadeInDown 0.2s ease-out'}}>
                    <div className="flex items-center gap-3.5 px-5 py-5">
                        <div className="w-[52px] h-[52px] rounded-full bg-gradient-to-br from-amber-500 to-zinc-500 text-white text-xl font-bold flex items-center justify-center shrink-0 shadow-[0_4px_16px_rgba(139,92,246,0.4)]">
                            {user?.name.charAt(0).toUpperCase()}
                        </div>
